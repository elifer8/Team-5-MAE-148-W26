...
TL_DETECTOR_NODE_NAME = 'traffic_light_detector_node'
TL_GATE_NODE_NAME = 'traffic_light_gate_node'
OAKD_SHARED_NODE_NAME = 'oakd_shared_node'
CONE_DEPTH_NODE_NAME = 'cone_depth_detector_node'
...
class Calibration(Node):
    ...
    def live_calibration_values(self, data):
        ...
        color_config_path = str(Path(__file__).resolve().parents[1] / 'config' / 'ros_racer_calibration.yaml')
        ...
        f.write(
            ...
            f"    cmd_vel_output_topic : /cmd_vel_nav \n"
            f"    lane_mode : center \n"
            f"    lane_mode_topic : /lane_mode \n"
            f"    left_lane_target_error : 0.2 \n"
            f"    right_lane_target_error : -0.2 \n"
            f"    avoidance_cmd_topic : /avoidance_cmd \n"
            f"    avoidance_throttle : 0.12 \n"
            f"    avoidance_left_steering : 0.6 \n"
            f"    avoidance_right_steering : -0.6 \n"
            f"    avoidance_left_duration : 1.0 \n"
            f"    avoidance_right_duration : 1.0 \n"
            f"    cone_distance_topic : /cone_distance_m \n"
            f"    cone_avoidance_enabled : true \n"
            f"    cone_trigger_distance_m : 2.0 \n"
            f"    cone_reset_distance_m : 2.5 \n"
            f"{TL_DETECTOR_NODE_NAME}: \n"
            f"  ros__parameters: \n"
            f"    camera_topic : /camera/color/image_raw \n"
            f"    depth_topic : /camera/depth/image_raw \n"
            f"    traffic_light_state_topic : /traffic_light/state \n"
            f"    traffic_light_distance_topic : /traffic_light/distance_m \n"
            f"    inference_server_url : http://localhost:9001 \n"
            f"    roboflow_model_id : traffic-light-training/6 \n"
            f"    api_key : '' \n"
            f"    image_path : /tmp/traffic_light_detector.jpg \n"
            f"    frame_skip : 1 \n"
            f"    confidence_threshold : 0.5 \n"
            f"    log_interval_sec : 1.0 \n"
            f"    state_hold_sec : 0.5 \n"
            f"    max_depth_age_sec : 0.2 \n"
            f"    depth_roi_scale : 2.5 \n"
            f"    min_depth_roi_size_px : 18 \n"
            f"    depth_percentile : 20.0 \n"
            f"    max_distance_m : 10.0 \n"
            f"{TL_GATE_NODE_NAME}: \n"
            f"  ros__parameters: \n"
            f"    input_cmd_topic : /cmd_vel_nav \n"
            f"    output_cmd_topic : /cmd_vel \n"
            f"    traffic_light_state_topic : /traffic_light/state \n"
            f"    stop_on_yellow : false \n"
            f"    traffic_light_distance_topic : /traffic_light/distance_m \n"
            f"    stop_distance_m : 4.0 \n"
            f"    distance_tolerance_m : 0.0 \n"
            f"    distance_timeout_sec : 1.0 \n"
            f"{OAKD_SHARED_NODE_NAME}: \n"
            f"  ros__parameters: \n"
            f"    camera_topic : /camera/color/image_raw \n"
            f"    depth_topic : /camera/depth/image_raw \n"
            f"    cone_distance_topic : /cone_distance_m \n"
            f"    inference_server_url : http://localhost:9001 \n"
            f"    roboflow_model_id : traffic-light-training/6 \n"
            f"    api_key : 'UOi4p4RmZsJV6kqNi8ln' \n"
            f"    image_path : /tmp/oakd_shared_cone_detector.jpg \n"
            f"    frame_skip : 2 \n"
            f"    confidence_threshold : 0.5 \n"
            f"    target_classes_csv : orange cone,cone \n"
            f"    log_interval_sec : 0.5 \n"
            f"    reconnect_interval_sec : 2.0 \n"
            f"    camera_fps : 15.0 \n"
            f"{CONE_DEPTH_NODE_NAME}: \n"
            f"  ros__parameters: \n"
            f"    inference_server_url : http://localhost:9001 \n"
            f"    roboflow_model_id : traffic-light-training/6 \n"
            f"    api_key : 'UOi4p4RmZsJV6kqNi8ln' \n"
            f"    image_path : /tmp/cone_depth_detector.jpg \n"
            f"    frame_skip : 2 \n"
            f"    confidence_threshold : 0.5 \n"
            f"    target_classes_csv : orange cone,cone \n"
            f"    distance_topic : /cone_distance_m \n"
            f"    log_interval_sec : 0.5 \n"
            f"    reconnect_interval_sec : 2.0 \n"
            f"{VESC_NODE_NAME}: \n"
            f"  ros__parameters: \n"
            ...
            f"    max_left_steering : {self.max_left_steering} \n"
            f"    vesc_reconnect_interval_sec : 2.0 \n"
            f"{ADA_NODE_NAME}: \n"
            f"  ros__parameters: \n"
            f"    steering_polarity : {self.steering_polarity} \n"
            f"    throttle_polarity : {self.throttle_polarity} \n"
