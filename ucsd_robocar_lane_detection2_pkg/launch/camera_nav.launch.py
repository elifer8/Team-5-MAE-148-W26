...
def generate_launch_description():
    ...
    traffic_light_detector_node = Node(
        package=lane_detection2_package,
        executable=tl_detector_node_name,
        output='screen',
        parameters=[config])

    traffic_light_gate_node = Node(
        package=lane_detection2_package,
        executable=tl_gate_node_name,
        output='screen',
        parameters=[config])

    ld.add_action(lane_detection_node)
    ld.add_action(lane_guidance_node)
    ld.add_action(traffic_light_detector_node)
    ld.add_action(traffic_light_gate_node)
    return ld
